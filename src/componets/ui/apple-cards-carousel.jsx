...
export const Card = ({ card, index, layout = false }) => {
  ...
  return (
    <>
        ...
        {open && (
          <div className="fixed inset-0 h-screen z-50 overflow-auto">
            ...
            <motion.div
              ...
              exit={{ opacity: 0 }}
              ref={containerRef}
              className="max-w-5xl mx-auto bg-white dark:bg-neutral-900 h-fit my-10 p-4 md:p-10 rounded-3xl relative"
            >
              <button
      ...
      <motion.button
        onClick={handleOpen}
        className="rounded-3xl bg-gray-100 dark:bg-neutral-900 h-80 w-56 md:h-[40rem] md:w-96 overflow-hidden relative flex flex-col justify-start"
        style={{
      ...
      >
        <div className=" inset-0 bg-gradient-to-b from-black/50 via-transparent to-transparent" />
        <div className="absolute top-4 left-0 p-4">
          <motion.p className="text-yellow-600 text-sm md:text-base font-medium">
            {card.category}
          </motion.p>
          <motion.p className="text-yellow-600 text-xl md:text-3xl font-semibold mt-2">
            {card.title}
          </motion.p>
        </div>
      </motion.button>
